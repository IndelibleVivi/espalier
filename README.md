...
`npm run check` includes typecheck, type-aware lint, package-boundary validation, all tests, the Web build, canonical Skill validation, and the tracked public-surface guard. GitHub Actions tests the exact Node 24.0.0 floor and the current Node 26 line with npm 11.19.1, rejects unreviewed dependency install scripts, runs pinned public-surface/security tooling, and records canonical/deterministic-contract coverage plus exact-commit review receipts. Required runtime evidence also exercises a real seeded Orchard Canvas in Chromium—including a token-authenticated mutation and SSE refresh—and the managed `seed → start → status → restart → stop` lifecycle on Ubuntu and macOS. The public repository separately uses GitHub-managed CodeQL default setup for JavaScript/TypeScript; it is a repository setting, not a duplicate workflow in this tree. These browser/runtime checks prove the named contract only; they do not replace production accessibility, performance, visual, release, deployment, or owner acceptance.
